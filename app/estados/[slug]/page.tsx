import type { Metadata } from "next";
import { notFound } from "next/navigation";
import Image from "next/image";
import Link from "next/link";
import { Header } from "@/components/header";
import { Breadcrumbs } from "@/components/breadcrumbs";
import { ContactButtons } from "@/components/contact-buttons";
import { FinalCta, Footer } from "@/components/cta-footer";
import { MobileCtaBar } from "@/components/mobile-cta-bar";
import { siteConfig } from "@/lib/site-config";

type Props = {
  params: Promise<{ slug: string }>;
};

function getState(slug: string) {
  return siteConfig.states.find((s) => s.slug === slug);
}

export function generateStaticParams() {
  return siteConfig.states.map((state) => ({ slug: state.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const state = getState(slug);

  if (!state) {
    return { title: "Estado no encontrado" };
  }

  const title = `Transporte de Carga a ${state.name}`;
  const description = state.metaDescription;

  return {
    title,
    description,
    alternates: {
      canonical: `/estados/${state.slug}`,
    },
    openGraph: {
      title,
      description,
      url: `${siteConfig.url}/estados/${state.slug}`,
      images: [
        {
          url: siteConfig.ogImage,
          width: 1200,
          height: 630,
          alt: `Transporte de carga a ${state.name}`,
        },
      ],
    },
  };
}

function StateJsonLd({ state }: { state: NonNullable<ReturnType<typeof getState>> }) {
  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "Service",
    serviceType: "Transporte de carga foráneo",
    name: `Transporte de carga a ${state.name}`,
    description: state.intro,
    provider: {
      "@type": "LocalBusiness",
      name: siteConfig.name,
      telephone: `+52${siteConfig.phone}`,
      url: siteConfig.url,
    },
    areaServed: {
      "@type": "State",
      name: state.name,
    },
  };

  return (
    <script
      type="application/ld+json"
      dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }}
    />
  );
}

export default async function StatePage({ params }: Props) {
  const { slug } = await params;
  const state = getState(slug);

  if (!state) {
    notFound();
  }

  const otherStates = siteConfig.states.filter((s) => s.slug !== state.slug);
  const waMessage = `Hola, necesito cotizar un flete a ${state.name}`;

  return (
    <>
      <StateJsonLd state={state} />
      <Header />
      <main className="pb-16 sm:pb-0">
        <Breadcrumbs items={[{ label: `Transporte a ${state.name}`, href: `/estados/${state.slug}` }]} />

        {/* Hero del estado */}
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-0">
          <div className="flex flex-col justify-center px-[6%] py-10 lg:py-16">
            <div className="inline-flex items-center gap-2 bg-azul-light text-azul text-xs font-semibold uppercase tracking-wide px-3.5 py-1.5 rounded-full mb-5 w-fit">
              Cobertura nacional
            </div>

            <h1 className="font-display text-[40px] sm:text-[52px] lg:text-[60px] leading-[0.95] text-gray-900 mb-4">
              TRANSPORTE DE CARGA A <span className="text-rojo">{state.name.toUpperCase()}</span>
            </h1>

            <p className="text-[16px] text-gris max-w-[520px] mb-6 leading-relaxed">
              {state.intro}
            </p>

            <div className="flex gap-5 flex-wrap mb-8 text-sm">
              <div className="flex items-center gap-2">
                <span className="font-bold text-gray-900">📍 Capital:</span>
                <span className="text-gris">{state.capital}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="font-bold text-gray-900">🛣️ Distancia:</span>
                <span className="text-gris">{state.km} km desde CDMX aprox.</span>
              </div>
            </div>

            <ContactButtons waMessage={waMessage} location="estado_hero" />
          </div>

          <div className="relative min-h-[280px] lg:min-h-full overflow-hidden bg-gray-100">
            <Image
              src="/images/hero-np300-ruta.jpg"
              alt={`Unidad de Transportes León rumbo a ${state.name}`}
              fill
              sizes="(max-width: 1024px) 100vw, 50vw"
              className="object-cover"
              priority
            />
          </div>
        </section>

        {/* Industrias que atendemos en el estado */}
        <section className="py-14 px-[6%] bg-gris-claro">
          <h2 className="font-display text-[28px] sm:text-[36px] text-gray-900 leading-none mb-8">
            SECTORES QUE ATENDEMOS EN {state.name.toUpperCase()}
          </h2>
          <div className="flex flex-wrap gap-3">
            {state.industries.map((industry) => (
              <span
                key={industry}
                className="bg-white border border-borde px-5 py-3 text-sm font-semibold text-gray-900 rounded-lg"
              >
                {industry}
              </span>
            ))}
          </div>
        </section>

        {/* Unidades disponibles para esta ruta */}
        <section className="py-14 px-[6%]">
          <h2 className="font-display text-[28px] sm:text-[36px] text-gray-900 leading-none mb-8">
            UNIDADES DISPONIBLES PARA {state.name.toUpperCase()}
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {siteConfig.units.map((unit) => (
              <div
                key={unit.slug}
                className="bg-white border border-borde rounded-xl overflow-hidden"
              >
                <div className="h-[160px] relative overflow-hidden">
                  <Image
                    src={unit.image}
                    alt={`${unit.name} disponible para flete a ${state.name}`}
                    fill
                    sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw"
                    className="object-cover"
                  />
                </div>
                <div className="p-4">
                  <div className="text-[15px] font-bold text-gray-900">
                    {unit.name}
                  </div>
                  <div className="text-xs text-gris mt-0.5">{unit.capacity}</div>
                </div>
              </div>
            ))}
          </div>
        </section>

        {/* Otros estados */}
        <section className="py-14 px-[6%] bg-gris-claro">
          <h2 className="font-display text-[28px] sm:text-[36px] text-gray-900 leading-none mb-8">
            OTROS ESTADOS QUE CUBRIMOS
          </h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
            {otherStates.map((s) => (
              <Link
                key={s.slug}
                href={`/estados/${s.slug}`}
                className="bg-white border border-borde rounded-lg px-4 py-3.5 flex justify-between items-center hover:border-rojo hover:shadow-md transition-all"
              >
                <span className="text-sm font-semibold text-gray-900">
                  → {s.name}
                </span>
                <span className="text-rojo text-lg font-bold">›</span>
              </Link>
            ))}
          </div>
        </section>

        <FinalCta />
      </main>
      <Footer />
      <MobileCtaBar />
    </>
  );
}
